...
def test_image_get_dockerfile_path():
    ...
    assert image_get_dockerfile_path(
        "samples/sample01", "portainer"
...
def test_image_load_in_memory():
    ...
    assert (
        image_load_in_memory("samples/sample01", "portainer")
        == "FROM portainer/portainer:latest\n"
    )
    assert (
        image_load_in_memory("samples/sample01", "app/hello-world")
