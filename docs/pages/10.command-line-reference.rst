...
Restart multiple configurations. You can force the restart (``-f/--force``), and restart in foreground (``--foreground``).
...
.. code-block:: bash
    ...
    # Restart the `test` configuration with force and in foreground
    docknv bundle restart -f --foreground test
    # same as 'docknv config use test; docknv config restart -f --foreground'
